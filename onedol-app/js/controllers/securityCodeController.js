define(function () {  
    'use strict';  
    function ctrl($scope,$controller,$mainServices,$interval,$http,$sce) {
    	$scope.data = {canNext: true, canSend: false};
    	$scope.mobileSmsLi = '';
    	$scope.smsBtn ="获取验证码";
        ...
        if (registerMobileLocal != null && registerMobileLocal.length == 11) {
			$scope.mobileSmsLi = $sce.trustAsHtml('<span style="color:#f00">请先滑动验证后，</span>验证码将发送至' + $mainServices.substringPhone(registerMobileLocal));
        }
    ...
    ctrl.$inject = ['$scope', '$controller','$mainServices','$interval','$http','$sce'];
    return ctrl;  
